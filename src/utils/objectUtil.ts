...
export const emptyStringToUndefined = <T>(
  obj: T | string
): T | string | undefined => {
...
export const undefinedToEmptyString = <T>(
  ...
  if (obj === undefined) {
    ...
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => [
        key,
        value === undefined ? '' : undefinedToEmptyString(value),
      ])
    ) as T;
  }
  return obj;
...
export const createEmptyObject = <T extends ZodRawShape>(
  ...
  Object.keys(schema.shape).reduce((result, key) => {
...
export const isNewRecord = (object: any): boolean => {
...
export const isEqual = <T extends string | number = string | number>(
  a: T[],
  b: T[]
) => {
  let i = a.length;
